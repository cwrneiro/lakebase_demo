...
28. **`BUNDLE_VAR_catalog_name` and `BUNDLE_VAR_lakebase_database_resource_id` couplings on raw `databricks bundle deploy`** — discovered 2026-04-29 during Wave 3 verification. `bootstrap.sh` plumbs `--catalog` to `BUNDLE_VAR_catalog_name` and looks up the live database resource id between passes, but anyone running `databricks bundle deploy -t dev` directly (e.g. for a code-only redeploy after a notebook edit) silently inherits the bundle defaults — `catalog_name=main` (wrong catalog) and `lakebase_database_resource_id=PLACEHOLDER_DATABASE_RESOURCE_ID` (overwrites the App resource id with the placeholder, breaking the running app). Fix candidates: (a) drop the `default: main` and `default: PLACEHOLDER_…` from `databricks.yml` so missing vars fail fast; (b) document the env-var requirement in README under "Local development"; (c) add a thin `scripts/bundle-deploy.sh` wrapper that always sources the right vars. Until fixed, the safe pattern is `BUNDLE_VAR_catalog_name=<your-catalog> BUNDLE_VAR_lakebase_database_resource_id=$(./scripts/get-db-resource-id.sh) databricks bundle deploy -t dev`.
29. **LightGBM model on synthetic data has AUC ≈ 0.5 → degenerate score distribution** — discovered 2026-04-29. The synthetic-data label (`subscriptions.status == 'churned'`) is independent of the features, so the trained model's probability outputs are tightly clustered (max raw `prob * 100` was ~36, putting all 5000 users in the `low` tier and emptying the `high` tier the UI is built around). Worked around in `pipelines/03_score_users.py` by mapping raw probabilities to *percentile-rank* scores: the model still drives the ordering (highest-percentile = top-predicted-risk), but the score range fills 0-100 so the UI tiers stay populated. When real data with calibrated probabilities arrives later, swap percentile-rank back for `prob * 100`. The `train_churn_model` task could also assert `auc > 0.55` to fail loudly when the data degenerates — left out for now since the demo intentionally uses synthetic data.

## Functional follow-ups (the demo works without these but they're tempting)

16. ~~**Replace heuristic scorer with a real model**~~ — **done + verified end-to-end 2026-04-29 on `<your-workspace-profile>`**. New notebook `pipelines/03a_train_churn_model.py` trains a LightGBM binary classifier on `${catalog}.${schema_prefix}_curated.user_features`, logs to MLflow with an explicit `infer_signature(...)` (required by UC), and registers under `${catalog}.${schema_prefix}_curated.churn_model` with the `champion` alias. `pipelines/03_score_users.py` loads the registered model via `mlflow.lightgbm.load_model` (raw Booster — `pyfunc` rejects pandas `category` dtype even when the model was trained on it), predicts, percentile-ranks the probabilities, and clips to [0, 100]. Closed-loop decay and the recommendations table are unchanged downstream. `resources/jobs.yml` has a `train_churn_model` task between `compute_features` and `score_users`. The honest caveat: the synthetic-data label is independent of features, so the AUC on the synthetic feature set is ~0.50; the demo value is the architecture (training notebook → MLflow → UC registry → alias load → percentile-ranked scoring), not the model's quality. A production fork retrains on actual historical churn outcomes; once probabilities are calibrated, swap percentile-rank back for `prob * 100` (one line in `03_score_users.py`). See #29.
